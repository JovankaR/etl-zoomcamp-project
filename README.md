...
- **Terraform** must be installed on your machine. [Install Terraform](https://developer.hashicorp.com/terraform/tutorials/aws-get-started/install-cli)
- **Google Cloud SDK** must be installed. [Install Google Cloud SDK](https://cloud.google.com/sdk/docs/install)
- Authenticate your Google Cloud account by running:
	```bash
	gcloud auth application-default login
	```
	This will allow Terraform and other tools to access your GCP resources.
...
> **Important:** Before running Docker Compose for the first time, you may need to initialize the Airflow database. Run the following command inside the `airflow-docker` directory:
>
> ```bash
> docker compose run airflow-webserver airflow db init
> ```
>
> This step is required only once, before the first startup.

After initializing the database, start the Airflow and related services:
...
> **Note:** The first time you run `docker-compose up`, it may take several minutes to complete because all required dependencies and images need to be installed and set up.
...
# CLUSTER_NAME may stay as the default from the .env file unless you need a custom cluster name
CLUSTER_NAME=default-from-env
BUCKET_NAME=default-from-terraform
BQ_DATASET=default-from-terraform
# GCS_SPARK_FOLDER may stay as the default from the .env file unless you need a custom folder
GCS_SPARK_FOLDER=default-from-env
